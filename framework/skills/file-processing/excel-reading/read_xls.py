...
使用 xlrd 库读取 Excel 97-2003 格式文件，提取工作表内容并返回结构化数据。
"""
...
def read_xls(
    ...
    if file_path.suffix.lower() not in ['.xls']:
        raise ValueError(f"不支持的文件格式: {file_path.suffix}，仅支持 .xls 文件")
        ...
        if "xlsx" in str(e).lower():
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    workbook_xlsx = load_workbook(filename=str(temp_xlsx), read_only=True, data_only=True)
                    
                    result: Dict[str, Any] = {
                        "file_info": {
                            "file_path": str(file_path),
                            "total_sheets": len(workbook_xlsx.sheetnames),
                            "read_sheets": 0,
                            "format": ".xls (实际为xlsx格式)",
                            "note": "文件扩展名为.xls但实际格式为xlsx"
                        },
                        "sheets": []
                    }
                    
                    target_sheets = sheet_names if sheet_names else workbook_xlsx.sheetnames
                    
                    for sheet_name in target_sheets:
                        if sheet_name not in workbook_xlsx.sheetnames:
                            workbook_xlsx.close()
                            raise ValueError(f"工作表不存在: {sheet_name}")
                        
                        worksheet = workbook_xlsx[sheet_name]
                ...
                raise ValueError(f"无法打开文件 (xlrd错误: {e}, openpyxl错误: {e2})")
        else:
        ...
        for row_idx in range(worksheet.nrows):
            ...
            for col_idx in range(worksheet.ncols):
                ...
                if cell.ctype == xlrd.XL_CELL_TEXT:
                    ...
                    row_list.append(bool(cell_value))
                elif cell.ctype == xlrd.XL_CELL_ERROR:
                    row_list.append("")
                elif cell.ctype == xlrd.XL_CELL_BLANK:
                    row_list.append("")
                else:
                    row_list.append(str(cell_value) if cell_value else "")
...
def read_xls_as_markdown(
    file_path: Union[str, Path],
    sheet_names: Optional[List[str]] = None,
    max_rows: Optional[int] = None
) -> str:
    """
    读取 xls 文件并返回 Markdown 格式的表格。
    ...
    Returns:
        Markdown 格式的字符串
    """
    data = read_xls(file_path, sheet_names, max_rows)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("用法: python read_xls.py <文件路径> [工作表名称...] [--max-rows N]")
        print("示例: python read_xls.py data.xls Sheet1 Sheet2 --max-rows 100")
        sys.exit(1)
    ...
    try:
        result = read_xls(file_path_arg, sheet_names_arg, max_rows_arg)
